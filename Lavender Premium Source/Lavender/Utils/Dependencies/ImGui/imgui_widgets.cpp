...
bool ImGui::SolTab(bool selected, const char* icon, const char* label, const char* description, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, xGetColorU32(c::tab::tab_active, state.bg_alpha), c::elements::rounding);

    RenderTextColor(font::lexend_regular, bb.Min + ImVec2(size.y, 0), bb.Max, GetColorU32(c::elements::text), description, ImVec2(0.0, 0.7));

    GetWindowDrawList()->xAddRectFilledMultiColor(bb.Min + ImVec2(size.x / 2, 0), bb.Max, xGetColorU32(state.shadow, 0.f), xGetColorU32(state.shadow, 1.f), xGetColorU32(state.shadow, 1.f), xGetColorU32(state.shadow, 0.f), 4);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec4 checkbox_gradient_start = ImColor(251, 28, 47, 255);
    ImVec4 checkbox_gradient_end = ImColor(110, 40, 40, 255);

    ImVec4 button_push_gradient_active = ImColor(251, 28, 47, 255);
    ImVec4 button_pop_gradient_active = ImColor(110, 40, 40, 255);

    ImVec4 button_push_gradient_hovered = ImColor(30, 29, 34, 0);
    ImVec4 button_pop_gradient_hovered = ImColor(19, 19, 19, 0);

    ImVec4 button_push_gradient = ImColor(251, 28, 47, 255);
    ImVec4 button_pop_gradient = ImColor(110, 40, 40, 255);

    ImVec4 text_active = ImColor(255, 255, 255, 255);
    ImVec4 text_hov = ImColor(100, 100, 100, 255);
    ImVec4 text = ImColor(175, 175, 175, 255);
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, id, &hovered, &held, flags);

    it_anim->second.background_color0 = ImLerp(it_anim->second.background_color0, IsItemActive() ? button_push_gradient_active : hovered ? button_push_gradient_hovered : button_push_gradient, g.IO.DeltaTime * 6.f);
    it_anim->second.background_color1 = ImLerp(it_anim->second.background_color1, IsItemActive() ? button_pop_gradient_active : hovered ? button_pop_gradient_hovered : button_pop_gradient, g.IO.DeltaTime * 6.f);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? text_active : hovered ? text_hov : text, g.IO.DeltaTime * 6.f);
    it_anim->second.alpha = ImLerp(it_anim->second.alpha, (hovered || held || pressed) ? 1.f : 0.f, g.IO.DeltaTime * 10.f);
    RenderFrame(bb.Min, bb.Max, GetColorU32(c::elements::background), false, c::elements::rounding);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImVec4{ c::accent.x,c::accent.y,c::accent.z,it_anim->second.alpha }), c::elements::rounding, 0);
...
bool ImGui::CheckboxNew(const char* label, const char* description, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    GetWindowDrawList()->xAddRectFilledMultiColor(rect.Min, rect.Max - ImVec2(20, 0), xGetColorU32(c::elements::background, 0.f), xGetColorU32(c::elements::background, 1.f), xGetColorU32(c::elements::background, 1.f), xGetColorU32(c::elements::background, 0.f), c::elements::rounding);

    GetWindowDrawList()->AddRect(rect.Max - ImVec2(37, 37), rect.Max - ImVec2(13, 13), GetColorU32(state.background), 2.f);
    GetWindowDrawList()->AddRectFilled(rect.Max - ImVec2(37, 37), rect.Max - ImVec2(13, 13), xGetColorU32(state.background, 0.5f), 2.f);

    PushClipRect(rect.Max - ImVec2(37, 37), rect.Max - ImVec2(13, 13), true);
    RenderCheckMark(GetWindowDrawList(), rect.Max - ImVec2(35 - (square_sz / 2) / 2, 35 - (square_sz / 2 + state.mark_pos) / 2), xGetColorU32(c::elements::mark, state.alpha), square_sz / 2);
    PopClipRect();
...
static const char* xItems_ArrayGetter(void* data, int idx)
{
...
bool ImGui::BeginCombo(const char* label, const char* description, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    GetWindowDrawList()->xAddRectFilledMultiColor(bb.Min, bb.Max - ImVec2(120, 0), xGetColorU32(c::elements::background, 0.f), xGetColorU32(c::elements::background, 1.f), xGetColorU32(c::elements::background, 1.f), xGetColorU32(c::elements::background, 0.f), c::elements::rounding);
    ...
    GetWindowDrawList()->xAddRectFilledMultiColor(rect.Min, rect.Max - ImVec2(30, 0), xGetColorU32(c::elements::background_widget, 0.f), xGetColorU32(c::elements::background_widget, 1.f), xGetColorU32(c::elements::background_widget, 1.f), xGetColorU32(c::elements::background_widget, 0.f), 2.f);
...
static const char* xxItems_SingleStringGetter(void* data, int idx)
{
    const char* items_separated_by_zeros = (const char*)data;
    int items_count = 0;
    const char* p = items_separated_by_zeros;
...
bool ImGui::Combo(const char* label, const char* description, int* current_item, const char* items_separated_by_zeros, int height_in_items)
{
    int items_count = 0;
    const char* p = items_separated_by_zeros;
    while (*p)
    ...
    bool value_changed = Combo(label, description, current_item, xxItems_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items);
    return value_changed;
}

bool ImGui::Combo(const char* label, const char* description, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int popup_max_height_in_items)
{
    ImGuiContext& g = *GImGui;
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        PushID(i);
        const bool item_selected = (i == *current_item);
        if (ImGui::Selectable(item_text, item_selected) && *current_item != i)
        {
            value_changed = true;
            *current_item = i;
        }
...
bool ImGui::Combo(const char* label, const char* description, int* current_item, const char* const items[], int items_count, int height_in_items)
{
    const bool value_changed = Combo(label, description, current_item, xItems_ArrayGetter, (void*)items, items_count, height_in_items);
    return value_changed;
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    bool hovered = ItemHoverable(frame_bb, id, flags), held, pressed = ButtonBehavior(frame_bb, id, &hovered, &held, NULL);
    bool temp_input_is_active = temp_input_allowed && TempInputIsActive(id);
    ...
    GetWindowDrawList()->xAddRectFilledMultiColor(rect.Min, rect.Max - ImVec2(120, 0), xGetColorU32(c::elements::background, 0.f), xGetColorU32(c::elements::background, 1.f), xGetColorU32(c::elements::background, 1.f), xGetColorU32(c::elements::background, 0.f), 4);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool hovered = ItemHoverable(frame_bb, id, item_status_flags);
    if (hovered)
   ...
   // const bool input_requested_by_nav = (g.ActiveId != id) && ((g.NavActivateInputId == id) || (g.NavActivateId == id && g.NavInputSource == ImGuiInputSource_Keyboard));
    ...
    PushFont(font::lexend_bold);
    {
        ...
        if (render_cursor || render_selection)
        {
            ...
            if (render_selection)
            {
                ...
                ImU32 bg_color = xGetColorU32(c::accent, 0.1f); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
                ...
                for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
                {
                    ...
                    else
                    {
                        ...
                        if (rect.Overlaps(clip_rect))
                            draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(3, 0), rect.Max + ImVec2(7, 0), bg_color);
                    }
            ...
            if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
            {
                ...
                draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll + ImVec2(5, 0), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
            }
            ...
            if (render_cursor)
            {
                ...
                if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) draw_window->DrawList->AddLine(ImVec2(total_bb.Min.x + state_anim.slow + 5, cursor_screen_rect.Min.y), ImVec2(total_bb.Min.x + state_anim.slow + 5, cursor_screen_rect.GetBL().y), GetColorU32(c::elements::text_active));
        ...
        else
        {
            ...
            {
                ImU32 col = GetColorU32(is_displaying_hint ? c::elements::text : c::elements::text_active);
                draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos + ImVec2(5, 0), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
            }
        }
    }
    PopFont();
    if (is_password && !is_displaying_hint)
        PopFont();
   // GetWindowDrawList()->xAddRectFilledMultiColor(frame_bb.Min + ImVec2(1, 1), frame_bb.Max - ImVec2(size_arg.y - CalcTextSize(hint).y - 1, 1), xGetColorU32(c::elements::background, 0.f), xGetColorU32(c::elements::background, 1.f), xGetColorU32(c::elements::background, 1.f), xGetColorU32(c::elements::background, 0.f), c::elements::rounding);
...
bool icon_box(const char* icon, ImVec2 size, ImU32 color_bg, ImU32 color_icon, ImU32 color_border)
{
    ...
    bool hovered, held, pressed = ImGui::ButtonBehavior(rect, id, &hovered, &held, NULL);

    static std::map<ImGuiID, iconbox_state> anim;
    iconbox_state& state = anim[id];

    ImGui::GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, ImGui::GetColorU32(color_bg), c::elements::rounding);
    ImGui::GetWindowDrawList()->AddRect(rect.Min, rect.Max, ImGui::GetColorU32(color_border), c::elements::rounding);

    RenderTextColor(font::icomoon_widget, rect.Min, rect.Max, ImGui::GetColorU32(color_icon), icon, ImVec2(0.5, 0.5));
...
bool color_button(const char* name, ImVec2 size, ImU32 color_bg)
{
    ...
    ImGui::GetWindowDrawList()->AddCircleFilled(rect.Min + ImVec2(size / 2), size.x / 2, ImGui::GetColorU32(color_bg), 30.f);

    ImGui::GetWindowDrawList()->AddCircle(rect.Min + ImVec2(size / 2), size.x / 3, ImGui::xGetColorU32(c::elements::background, 0.3f), 30.f, 3.f);
...
bool xColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
    ...
    if (!(flags & ImGuiColorEditFlags_NoOptions)) ImGui::OpenPopupOnItemClick("context", ImGuiPopupFlags_MouseButtonRight);
    ...
    ImU32 user_col32_striped_of_alpha = ImGui::ColorConvertFloat4ToU32(ImVec4(R, G, B, style.Alpha));

    static std::map<ImGuiID, xpicker_state> anim;
    xpicker_state& state = anim[ImGui::GetID(label)];

    ImVec2 sv_cursor_pos;

    draw_list->xAddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), col_white, hue_color32, hue_color32, col_white, 5.f);
    draw_list->xAddRectFilledMultiColor(picker_pos - ImVec2(1, 1), picker_pos + ImVec2(sv_picker_size + 1, sv_picker_size + 1), 0, 0, col_black, col_black, 5.f);
    ...
    for (int i = 0; i < 6; ++i) draw_list->xAddRectFilledMultiColor(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6) - (i == 5 ? 1 : 0)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6) + (i == 0 ? 1 : 0)), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1], 10.f, i == 0 ? ImDrawFlags_RoundCornersTop : i == 5 ? ImDrawFlags_RoundCornersBottom : ImDrawFlags_RoundCornersNone);
    ...
    if (alpha_bar)
    {
        ...
        draw_list->xAddRectFilledMultiColor(bar1_bb.Min, bar1_bb.Max, user_col32_striped_of_alpha, user_col32_striped_of_alpha, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, 100.f);
    ...
    if (value_changed) ImGui::MarkItemEdited(g.LastItemData.ID);
...
bool xColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    if (!ImGui::ItemAdd((flags & ImGuiColorEditFlags_NoLabel) ? clickable : rect, id)) return false;

    bool hovered, held, pressed = ImGui::ButtonBehavior(clickable, id, &hovered, &held);

    if (flags & ImGuiColorEditFlags_NoAlpha) flags &= ~(ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf);

    ImVec4 col_rgb = col;
    if (flags & ImGuiColorEditFlags_InputHSV) ImGui::ColorConvertHSVtoRGB(col_rgb.x, col_rgb.y, col_rgb.z, col_rgb.x, col_rgb.y, col_rgb.z);

    ImGui::GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, ImGui::GetColorU32(c::elements::background), c::elements::rounding);

    ImGui::GetWindowDrawList()->AddRectFilled(clickable.Min, clickable.Max, ImGui::GetColorU32(col_rgb), c::elements::rounding);

    ImGui::RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min, clickable.Max, ImGui::GetColorU32(col_rgb), ImMin(20, 20) / 2.99f, ImVec2(0.f, 0.f), c::elements::rounding);

    return pressed;
}
#include <string>
bool ImGui::ColorEdit4New(const char* label, const char* description, float col[4], ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        const ImVec4 col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);
        if (xColorButton("##ColorButton", col_v4, flags, ImVec2(20, 20)))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                g.ColorPickerRef = col_v4;
        ...
        if (ItemHoverable(g.LastItemData.Rect, g.LastItemData.ID,flags) && g.IO.MouseClicked[0] || state.active && !search_col && g.IO.MouseClicked[0] && !state.hovered)
        ...
        if (state.active)
        {
            ...
            Begin("picker", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize);
            {
                ...
                value_changed |= xColorPicker4("##picker", col, picker_flags, &g.ColorPickerRef.x);



                if (InputTextEx("v", "HEX COLOR", buf, IM_ARRAYSIZE(buf), ImVec2(!(flags & ImGuiColorEditFlags_NoAlpha) ? 178 : 168, 35), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
                {
                    value_changed = true;
                    char* p = buf;
                ...
                if (icon_box(add_status ? "g" : "k", ImVec2(35, 35), GetColorU32(c::elements::background_widget), GetColorU32(c::accent), xGetColorU32(c::accent, 0.f)))
                {
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max + ImVec2(0, 1), xGetColorU32(c::elements::background_widget, state.opticaly), 2.f);
