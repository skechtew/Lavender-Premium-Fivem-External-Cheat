...
namespace FrameWork
{
	HWND Memory::GetWindowHandleByPID(DWORD Pid)
	{
		...
		HandleData.Pid = Pid;
		HandleData.WindowName = xorstr_(L"");
	...
	DWORD Memory::GetProcessPidByName(std::wstring ProcessName)
	{
		...
		{
#ifdef _DEBUG
			std::cout << xorstr_("[ERROR : FrameWork::Memory::GetProcessPidByName::CreateToolhelp32Snapshot] Error:") << SafeCall(GetLastError)() << std::endl;
#endif // _DEBUG
			return 0;
		}
	...
	uint64_t Memory::GetModuleBaseByName(DWORD Pid, std::wstring ModuleName)
	{
		...
		{
#ifdef _DEBUG
			std::cout << xorstr_("[ERROR : FrameWork::Memory::GetModuleBaseByName::CreateToolhelp32Snapshot] Error:") << SafeCall(GetLastError)() << std::endl;
#endif // _DEBUG
			return 0;
		}
	...
	void Memory::AttachProces(DWORD Pid)
	{
		...
		k_Kekeler.pubHANDLE = AttachedProcessHandle;
